...
static uint8_t own_addr_type; // Menyimpan tipe alamat BLE yang valid

BleReceiver::BleReceiver() {}

void BleReceiver::StartAdvertising() {
    // 1. Set payload paket advertising (agar nama perangkat dapat dibaca ESP32-C3)
    ...
    // 2. Konfigurasi parameter advertising
    struct ble_gap_adv_params adv_params;
    memset(&adv_params, 0, sizeof(adv_params));
    adv_params.conn_mode = BLE_GAP_CONN_MODE_UND;
    adv_params.disc_mode = BLE_GAP_DISC_MODE_GEN;

    // 3. Mulai advertising menggunakan own_addr_type hasil infer
...
void BleReceiver::OnSync() {
    ...
    // PERBAIKAN: Gunakan pointer &own_addr_type, JANGAN diisi NULL
